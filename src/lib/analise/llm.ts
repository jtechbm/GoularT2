...
/**
 * A análise da loja escrita por modelo de linguagem.
 *
  ...
  properties: {
    ...
    problemas: {
      ...
      items: {
        ...
        required: ["titulo", "evidencia", "gravidade"],
        additionalProperties: false,
      },
    },
    acoes: {
      ...
      items: {
        ...
        required: ["titulo", "por_que", "impacto", "esforco", "prazo"],
        additionalProperties: false,
      },
    },
...
interface RespostaCrua {
  ...
  problemas: { titulo: string; evidencia: string; gravidade: string }[];
  acoes: { titulo: string; por_que: string; impacto: string; esforco: string; prazo: string }[];
  sem_resposta: string[];
}
...
function instrucoes(): string {
  return [
    "Você analisa a operação de uma loja em marketplace para a equipe de uma",
    "agência brasileira. Escreva em português do Brasil, direto, sem enrolação",
    "e sem elogio vazio.",
    "",
    "Você recebe um dossiê com os números reais da loja. Regras:",
    "",
    ...
    "3. As ações têm de ser específicas desta loja: nomes de produto, nome de",
    "   campanha, faixa de preço. 'Melhorar os anúncios' não serve.",
    ...
    "6. Cuidado com dois números que enganam: ACOS indefinido significa que",
    ...
    "",
    "O diagnóstico é um parágrafo: como a loja está e o que mais pesa agora.",
  ].join("\n");
}
 ...
 */
export async function analisarLoja(dossie: Dossie, orcamentoMs = ORCAMENTO_MS): Promise<ResultadoAnalise> {
  if (!process.env.OPENAI_API_KEY) {
    throw new Error("Falta OPENAI_API_KEY para a IA analisar a loja.");
  }
  ...
  try {
    const resposta = await client.responses.create({
      ...
      text: { format: { type: "json_schema", name: "analise_loja", schema: ESQUEMA, strict: true } },
    });
  ...
  let naoConferidos = 0;
  const conferir = (item: { titulo: string; evidencia: string; gravidade?: string }): ItemComEvidencia => {
    const conferido = evidenciaConfere(item.evidencia, numeros);
    if (!conferido) naoConferidos += 1;
    return { titulo: item.titulo, evidencia: item.evidencia, gravidade: item.gravidade, conferido };
  };
